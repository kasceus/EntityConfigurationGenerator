...
namespace EntityConfigurationGenerator
{
	...
	internal sealed class GenerateSingleConfigurationCommand
	{
		...
		private async void Execute(object sender, EventArgs e)
		{
			...
			{
				outputPane.OutputStringThreadSafe($"  Config directory already exists: {configDirectory}\n");
			}
			else
			{
			...
			var configPath = Path.Combine(configDirectory, $"{className}Configuration.cs");
...
{{
	...
	{{
		...
		{{
			...
			// Add to project
			ProjectItems parentItems = selectedItem.ProjectItem.ContainingProject.ProjectItems;
			ProjectItem configFolderItem = parentItems
				.Cast<ProjectItem>()
				.FirstOrDefault(i =>
				{
			...
			if (configFolderItem == null)
			{
				// Create the Configurations folder if it doesn't exist
				parentItems.AddFolder("Configurations");
				outputPane.OutputStringThreadSafe("Creating configurations in project");
